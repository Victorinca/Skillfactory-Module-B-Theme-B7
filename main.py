...
class BoardOutException(BoardException):
    def __str__(self):
        return 'Вы пытаетесь выстрелить за границы доски. \n' \
               'Нельзя выстрелить за пределы доски!'
...
class Board:
    def __init__(self, size: int, hid=False):
        ...
        self.busy = []
        self.ships = []
        self.last_hit = []          # список точек подбитого корабля
    ...
    def shot(self, d: Dot) -> bool: # возвращает True, если нужно ход изменить
        if self.out(d):
        ...
        print('Выстрел мимо! Нужно лучше прицеливаться!')
        return False
    ...
    def ask(self) -> Dot:
        ...
        while True:
            ...
            if d not in self.enemy.busy and not self.enemy.out(d):
                break
        sleep(0.1 * randint(15, 50)) # имитация думающего компьютера
...
class User(Player):
    def ask(self) -> Dot:
        while True:
            coords = input('Введите координаты выстрела через пробел в формате y x \t').split()
            if len(coords) != 2:
